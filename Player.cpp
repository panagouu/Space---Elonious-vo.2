...
void Player::init()
{
	m_pos_x = m_state->getCanvasWidth() / 3.0f;
	m_pos_y = m_state->getCanvasHeight() / 2.0f;

	/** 
	Initially set the width and the height for the player's box */
	m_width = 1.0f;
	m_height = 1.0f;
...
void Player::update(float dt)
{
	...
	if (graphics::getKeyState(graphics::SCANCODE_A)) { m_pos_x -= delta_time * 8.5f; }
	if (graphics::getKeyState(graphics::SCANCODE_D)) { m_pos_x += delta_time * 5.0f; }
	if (graphics::getKeyState(graphics::SCANCODE_W)) { m_pos_y -= delta_time * 8.0f; }
	if (graphics::getKeyState(graphics::SCANCODE_S)) { m_pos_y += delta_time * 8.0f; }
...
void Player::drawHealth()
{
	...
	{
		graphics::drawRect((m_state->getCanvasWidth() * 0.05f) + (i*0.35f), m_state->getCanvasHeight() * 0.035f, 0.35f, 0.35f, m_brush_lives);
	}
}
